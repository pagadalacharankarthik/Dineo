  ...
  return (
    <div className="space-y-6 text-zinc-900 dark:text-zinc-100">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-zinc-800 dark:text-zinc-100">Restaurant Operations</h1>
          <p className="text-zinc-500 dark:text-zinc-400 text-sm mt-1">Manage tenant accounts, subscriptions, & toggle restrictions.</p>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input
            ...
            className="pl-10 bg-white dark:bg-zinc-900/40 border-zinc-200 dark:border-zinc-800 focus:border-red-500/50 focus:ring-red-500/20 text-zinc-800 dark:text-zinc-100 placeholder:text-zinc-400 dark:placeholder:text-zinc-650"
          />
        </div>
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
                    ...
                    <div className="space-y-3 flex-1">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="text-[10px] bg-orange-500/10 text-orange-600 dark:text-orange-400 border border-orange-500/20 font-mono px-2 py-0.5 rounded select-all font-bold" title="Unique Short ID">
                          Short ID: {rest.id.replace(/^cm/, "").substring(0, 8).toUpperCase()}
                        </span>
                        <span className="text-[10px] bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-550 dark:text-zinc-400 font-mono px-2 py-0.5 rounded select-all" title="Full DB CUID ID">
                          Full ID: {rest.id}
                        </span>
                        {isDel && (
                        ...
                        {isSusp && !isDel && (
                          <span className="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-400">
                            Suspended
                          </span>
                        )}
                      ...
                      <div className="grid gap-2 text-sm text-zinc-600 dark:text-zinc-400 md:grid-cols-2">
                        <div className="flex items-center gap-2">
                          <UserIcon className="w-4 h-4 text-zinc-400 dark:text-zinc-650" />
                      ...
                      <div className="mt-4 p-3 bg-zinc-50 dark:bg-zinc-900/60 rounded-lg border border-zinc-150 dark:border-zinc-850 flex flex-wrap items-center justify-between gap-4 text-xs">
                        ...
                        {rest.trialEndDate && (
                          <div className="space-y-1">
                            <div className="text-zinc-500 dark:text-zinc-450">Trial Expiration</div>
                            <div className="text-zinc-700 dark:text-zinc-300 font-medium">{new Date(rest.trialEndDate).toLocaleDateString()}</div>
                          </div>
                        )}
                        {rest.planExpiresAt && (
                          <div className="space-y-1">
                            <div className="text-zinc-500 dark:text-zinc-450">Billing Term</div>
                            <div className="text-zinc-700 dark:text-zinc-300 font-medium">{new Date(rest.planExpiresAt).toLocaleDateString()}</div>
                          </div>
                        )}
                        <Button 
                    ...
                    <div className="flex flex-row lg:flex-col items-center gap-2.5 justify-end lg:w-44 border-t lg:border-t-0 lg:border-l border-zinc-200 dark:border-zinc-800/80 pt-4 lg:pt-0 lg:pl-6">
                      {isDel ? (
                        <Button
                          onClick={() => handleAction(rest.id, "restore")}
                          variant="outline"
                          size="sm"
                          className="w-full flex items-center justify-center gap-2 text-zinc-700 dark:text-zinc-300 border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-850 text-xs font-semibold"
                        >
      ...
      <Dialog open={!!selectedRest} onOpenChange={() => setSelectedRest(null)}>
        <DialogContent className="bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 text-zinc-800 dark:text-zinc-100 max-w-md">
          <DialogHeader>
            <DialogTitle className="text-zinc-800 dark:text-zinc-150">Modify Subscription Model</DialogTitle>
          </DialogHeader>
          {selectedRest && (
            <form onSubmit={handleSaveSubscription} className="space-y-4 pt-2">
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-zinc-500 dark:text-zinc-400">Subscription Tier</label>
                <select
                  value={planName}
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-zinc-500 dark:text-zinc-400">Subscription Status</label>
                <select
                  value={planStatus}
                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setPlanStatus(e.target.value)}
                  className="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-md p-2 text-zinc-800 dark:text-zinc-100 focus:outline-none focus:border-red-500/50"
                >
                  <option value="ACTIVE" className="bg-white dark:bg-zinc-900 text-zinc-800 dark:text-zinc-100">Active</option>
                  <option value="EXPIRED" className="bg-white dark:bg-zinc-900 text-zinc-800 dark:text-zinc-100">Expired</option>
                  <option value="SUSPENDED" className="bg-white dark:bg-zinc-900 text-zinc-800 dark:text-zinc-100">Suspended</option>
                </select>
              </div>
              ...
              <div className="space-y-3 pt-2 border-t border-zinc-200 dark:border-zinc-800">
                <label className="block text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase">Dashboard Promotion Banners</label>
                <div className="grid grid-cols-1 gap-2">
                  <label className="flex items-center gap-2 p-2 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950/40 text-xs font-medium cursor-pointer text-zinc-700 dark:text-zinc-350">
                    <input
                      type="checkbox"
                      checked={showTrialBanner}
                      onChange={(e) => setShowTrialBanner(e.target.checked)}
                      className="rounded text-red-500 bg-zinc-55 dark:bg-zinc-900 border-zinc-300 dark:border-zinc-800 focus:ring-0 focus:ring-offset-0 cursor-pointer"
                    />
              ...
              <div className="flex justify-end gap-2.5 pt-4">
                <Button 
                  ...
                  className="border-zinc-200 dark:border-zinc-800 text-zinc-500 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-zinc-800 dark:hover:text-zinc-200"
                >
                  Discard
                </Button>
                <Button 
