...
export default function DashboardPage() {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: `Check out our digital menu on Dineo Menu!`,
          url: stats.publicUrl,
        });
      } catch (err) {
    ...
    try {
      ...
      if (ctx) {
        ...
        await new Promise<void>((resolve) => {
          ...
          logoImg.onerror = () => {
            console.warn("Failed to load custom logo, falling back to default Dineo Menu logo");
  ...
  const handleDownloadMenuPDF = async () => {
    ...
    try {
      ...
      // If custom logo failed or doesn't exist, try loading default Dineo Menu logo-light
      if (!logoData) {
        try {
          ...
          console.error("Failed to load default Dineo Menu logo for menu:", e);
        }
      }
      ...
      doc.text("Generated by Dineo Menu - Beautiful Digital Menus & QR Table Standees", 105, 290, { align: "center" });
      ...
      <div className="flex flex-col items-center justify-center min-h-[70vh] max-w-lg mx-auto text-center space-y-6">
        ...
        <div className="space-y-2">
          <h1 className="text-3xl font-extrabold tracking-tight">
            Welcome to Dineo Menu, {firstName}! 🚀
          </h1>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Here&apos;s a quick overview of your Dineo Menu QR Menu System.
          </p>
        </div>
        {stats?.publicUrl && (
          <a
